...
public class ResolveController {
    ...
    public String stringReturning(Model model) {
        ...
        *   ThymeleafViewResolver 에게 너가 앞에 resource/template 붙이고
    ...
    @GetMapping("string-redirect")      // redirect 는 간단히 바로~끝~
    public String stringRedirect() {

        /* 참고. 접두사로 redirect: 을 하면
    ...
    * */
    ...
    public String stringRedirectFlashAttr(RedirectAttributes rttr){
        ...
        *   세션에 임시로 값을 담고 소멸하는 방식이기 때문에 session 에 동일한 키값이 존재하면 안된다.
        * */
    ...
    @GetMapping("modelandview") // modelandview 에 대한 핸들러 만들기~~(main에서~)
    public ModelAndView modelandviewReturning(ModelAndView mv) {
        ...
        *   이 때 문자열을 반환해도 되지만, ModelAndView 를 만들어서 반환해도 된다. (선택사항~)
        * */

        mv.addObject("forwardMessage", "ModelAndView 를 이용한 모델과 뷰 반환");
        mv.setViewName("result");       // 이거 안하면 해당 url 이 알아서 딱 붙으며 오류난다!(ModelAndView로~~//해당탬플릿 찾을 수 없다면서~~)
    ...
    public ModelAndView modelAndViewRedirect(ModelAndView mv) {
        ...
        mv.setViewName("redirect:/");       // redirect 만나면 main 페이지로 redirect 시켜준 것이다!!

        return mv;

    }

    @GetMapping("modeldandview-redirect-attr")
    public ModelAndView modelAndViewRedirect(ModelAndView mv, RedirectAttributes rttr) {

        rttr.addFlashAttribute("flashMessage2","ModeAndView 를 이용한 redirect attr");
